...
cpiU <- read.xlsx("./data/SeriesReport-20160224001724_bf095b.xlsx", sheetName = "BLS Data Series", startRow = 11, header = TRUE, endRow = 51)
...
shinyServer(
  function(input, output) {
    ...
    output$od3 <- renderPrint({ 
      as.numeric(input$id3) - 2014 #Calculate number of years
      })
    output$od4 <- renderPrint({
      ...
      paste0("$", round(p, digits=2)) #Return inflated result
      })
    
    output$newPlot <- renderPlot({
        Y<-as.numeric(input$id3) - 2014 #Calculate number of years
        #Recalculate forecast for specific year input
